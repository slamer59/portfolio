...
export default function SharedModal({
	...
	const endIndex = Math.min(index + 15, images?.length || 0);
			...
			>
								...
								{currentImage && (
									<Image
										...
										src={urlFor(currentImage)
											.size(
												currentImage.dimensions?.width || 1920,
												currentImage.dimensions?.height || 1280,
											)
											.fit("scale")
											.url()}
										...
										width={
											...
											currentImage.dimensions?.aspectRatio
												? currentImage.dimensions.width *
													currentImage.dimensions.aspectRatio
												: 1920
										}
										/* @ts-ignore */
										height={currentImage.dimensions?.height || 1280}
										placeholder="blur"
										/* @ts-ignore */
					...
					{loaded && (
						<div className="relative aspect-[3/2] max-h-full w-full">
							...
							<div className="absolute top-0 right-0 flex items-center gap-2 p-3 text-white">
								{navigation ? (
									<a
										href={urlFor(currentImage).format("webp").url()}
                ...
                >
                  <ArrowDownTrayIcon className="w-5 h-5" />
					...
					{/* Photo Description */}
					{loaded && !navigation && galleryTitle && (
						<PhotoDescription
							photoTitle={currentImage?.title}
							photoAlt={currentImage?.alt}
							photoDescription={currentImage?.description}
							galleryTitle={galleryTitle}
							galleryDescription={galleryDescription}
							photoIndex={index}
							totalPhotos={images?.length || 1}
